...
const wordBooksCode=await wordBooksUi.text();
assert.match(wordBooksCode,/选择词书/);
assert.match(wordBooksCode,/parseWordBookJson/);
assert.match(wordBooksCode,/未收录于 ECDICT 的词也可以直接学习/);
const versionUi=await worker.fetch(new Request(origin+'/version.js'),env);
assert.equal(versionUi.status,200);
assert.match(await versionUi.text(),/1\.6\.1/);
