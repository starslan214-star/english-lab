...
const books=await readFile(new URL('worker/books.mjs',root),'utf8');
...
const bundledBooks=books.replace('export async function getOpenBook','async function getOpenBook').replace('export async function getWordBookArchive','async function getWordBookArchive');
const bundledHandler=handler.replace("import {getNews} from './news.mjs';",'').replace("import {getDictionaryDefinition,getDatamuseDefinition} from './dictionary.mjs';",'').replace("import {translateBaidu} from './baidu.mjs';",'').replace("import {getOpenBook,getWordBookArchive} from './books.mjs';",'').replace('export function createHandler','function createHandler');
