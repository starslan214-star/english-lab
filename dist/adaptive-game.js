...
function adaptiveGamePage(){
  ...
  const modes=[['meaning',adaptiveText('英选中','English → Chinese')],['reverse',adaptiveText('中选英','Chinese → English')],['listening',adaptiveText('听音选词','Listen & choose')],['spelling',adaptiveText('拼写','Spelling')],['context',adaptiveText('例句填空','Sentence gap')]];
  ...
  const spelling=adaptiveGameMode==='spelling',listening=adaptiveGameMode==='listening',context=adaptiveGameMode==='context';
  const example=question.target.ex||`Choose the word that means: ${question.target.zh}`;
  const gap=example.toLowerCase().includes(question.target.w.toLowerCase())?example.replace(new RegExp(question.target.w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'),'_____'):`_____ — ${question.target.zh}`;
  const prompt=adaptiveGameMode==='meaning'?`${adaptiveText('选择正确的中文意思','Choose the correct meaning')}: ${question.target.w}`:spelling?`${adaptiveText('根据释义拼写英文单词','Spell the English word')}: ${question.target.zh}`:listening?adaptiveText('听发音，选择你听到的单词','Listen and choose the word you hear'):context?`${adaptiveText('选择最适合句子的单词','Choose the best word for the sentence')}: ${gap}`:`${adaptiveText('选择对应的英文单词','Choose the matching English word')}: ${question.target.zh}`;
...
function recordAdaptiveResult(correct){
  const question=adaptiveGameQuestion,rating=correct?'good':'again',reward=['spelling','listening','context'].includes(adaptiveGameMode)?15:10;
