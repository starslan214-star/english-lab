...
function adaptiveGamePage(){
  ...
  const modes=[['meaning',adaptiveText('英选中','English → Chinese')],['reverse',adaptiveText('中选英','Chinese → English')],['spelling',adaptiveText('拼写','Spelling')]];
  ...
  const spelling=adaptiveGameMode==='spelling';
  const prompt=adaptiveGameMode==='meaning'?`${adaptiveText('选择正确的中文意思','Choose the correct meaning')}: ${question.target.w}`:spelling?`${adaptiveText('根据释义拼写英文单词','Spell the English word')}: ${question.target.zh}`:`${adaptiveText('选择对应的英文单词','Choose the matching English word')}: ${question.target.zh}`;
  ...
  const targetWord=spelling&&!adaptiveGameAnswered?'•'.repeat(Math.min(12,question.target.w.length)):question.target.w;
  const defaultNotice=adaptiveText(spelling?'输入后按回车提交，系统会自动安排复习。':'选择答案后，系统会自动更新该词的复习时间。',spelling?'Type your answer and press Enter to update the review schedule.':'Choose an answer to update this word’s review schedule.');
...
function recordAdaptiveResult(correct){
  const question=adaptiveGameQuestion,rating=correct?'good':'again',reward=adaptiveGameMode==='spelling'?15:10;
